...
interface College { id: string; name: string; state: string; city: string; ownership: Ownership; }
...
function generateColleges(): College[] {
  const kinds = ["Institute of Technology", "College of Engineering", "University", "Institute of Science", "Polytechnic", "Management Institute"];
  const result: College[] = [];
  let idx = 1;
  for (const [state, cities] of Object.entries(STATES)) {
    for (const city of cities) {
      for (let i = 0; i < 25; i++) { // ~25 per city
        const kind = kinds[(i + city.length) % kinds.length];
        const ownership = OWNERSHIPS[(i + idx) % OWNERSHIPS.length];
        result.push({ id: `c_${idx++}`, name: `${city} ${kind} ${i + 1}`, state, city, ownership });
      }
    }
  }
...
export default function Colleges() {
  ...
  useEffect(() => { setCity("all"); setPage(1); }, [state]);
  useEffect(() => { setPage(1); }, [q, own, city]);

  const citiesForState = useMemo(() => state === "all" ? [] : STATES[state] ?? [], [state]);

  const filtered = useMemo(() => {
    const ql = q.trim().toLowerCase();
    return ALL_COLLEGES.filter((c) =>
      (ql === "" || `${c.name} ${c.city} ${c.state}`.toLowerCase().includes(ql)) &&
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Browse {ALL_COLLEGES.length}+ colleges. Filter by state, city, and ownership.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Input placeholder="Search college or city" value={q} onChange={(e)=> setQ(e.target.value)} />
          ...
          <Select value={own} onValueChange={(v)=> setOwn(v as any)}>
            <SelectTrigger className="w-40"><SelectValue placeholder="Ownership" /></SelectTrigger>
            <SelectContent>
        ...
        {pageData.map((c) => (
          <Card key={c.id} className="transition hover:shadow-lg">
            <CardContent className="p-5 space-y-2">
              ...
              <div className="text-sm text-muted-foreground">{c.city}, {c.state}</div>
            </CardContent>
          </Card>
        ))}
      </div>
